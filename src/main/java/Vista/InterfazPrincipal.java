...
public class InterfazPrincipal extends JFrame {
    ...
    // Google Maps API Key (puedes usarla en peticiones HTTP si necesitas funcionalidades avanzadas)
    private final String googleMapsApiKey = "AIzaSyC0DvctsrETeLxifYjRzzFcA4uqkEp8Zvc";
    ...
    public InterfazPrincipal() {
        ...
        // Botón para mostrar Google Maps
        JButton botonMapa = crearBoton("Ver Mapa", new Color(0, 180, 90));
        botonMapa.setBounds(540, 440, 170, 40);
        panelInicio.add(botonMapa);

        botonMapa.addActionListener(e -> {
            String ubicacion = JOptionPane.showInputDialog(this, "¿Qué lugar quieres ver en el mapa?");
            if (ubicacion != null && !ubicacion.isBlank()) {
                abrirGoogleMaps(ubicacion);
            }
        });
    ...
    // Método para abrir Google Maps en el navegador
    private void abrirGoogleMaps(String ubicacion) {
        try {
            String url = "https://www.google.com/maps/search/?api=1&query=" + ubicacion.replace(" ", "+");
            Desktop.getDesktop().browse(new URI(url));
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "No se pudo abrir Google Maps.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
